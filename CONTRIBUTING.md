# Contributing to Ought

Thanks for your interest in contributing!

## License of contributions

Ought is currently distributed under the [MIT License](LICENSE).

By submitting a contribution (pull request, patch, or any other form) to this
project, you agree that:

1. Your contribution is licensed under the MIT License, the same terms as the
   rest of the project.
2. You have the right to submit the work under that license.
3. **You grant Sosein LLC the right to relicense your contribution under any
   other license, at its sole discretion, now or in the future.** This
   includes, but is not limited to, dual-licensing, relicensing under a
   different open-source license, or relicensing under a commercial license.

   This relicensing right is granted irrevocably and without requiring further
   notice or compensation. If you do not agree to these terms, please do not
   submit contributions to this project.

## How to contribute

- Open an issue to discuss substantial changes before starting work.
- Keep pull requests focused — one logical change per PR.
- Run `cargo build` and `cargo test` before submitting.
- Follow the existing code style.
