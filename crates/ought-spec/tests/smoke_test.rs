...
fn parse_real_spec_file() {
    let path = Path::new(env!("CARGO_MANIFEST_DIR")).join("../../ought/engine/parser.ought.md");
    if !path.exists() {
    ...
    let spec = OughtMdParser
        .parse_file(&path)
        .expect("failed to parse parser.ought.md");
    assert_eq!(spec.name, "Parser");
    assert!(
        spec.metadata.context.is_some(),
        "should have context metadata"
    );
    assert!(
        !spec.metadata.sources.is_empty(),
    ...
    assert!(
        total > 30,
        "parser spec should have many clauses, got {}",
        total
    );
    ...
    let _has_given_condition = spec.sections.iter().any(|s| {
        s.clauses.iter().any(|c| c.condition.is_some())
            || s.subsections
                .iter()
                .any(|sub| sub.clauses.iter().any(|c| c.condition.is_some()))
    });
