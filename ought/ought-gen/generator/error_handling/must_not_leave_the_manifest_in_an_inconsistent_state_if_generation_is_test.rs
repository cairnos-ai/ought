...
fn test_generator__error_handling__must_not_leave_the_manifest_in_an_inconsistent_state_if_generation_is() {
    ...
    // (we never call manifest.save again — simulates ctrl+c or panic after generate()
    //  but before the manifest write)
    ...
    assert!(
        ...
        "must_not_leave_manifest_inconsistent: committed clause_a must be present in loaded manifest"
    );
    assert!(
    ...
    // Each present entry must be fully populated — no empty/partial fields
    ...
    // A manifest that already existed must survive a second load (idempotent)
