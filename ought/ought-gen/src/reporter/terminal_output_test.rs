...
fn make_result_with_failure_detail(
    clause_id: &str,
    status: TestStatus,
    failure_msg: &str,
) -> TestResult {
...
fn test_reporter__terminal_output__must_show_each_clause_with_keyword_text_and_status() {
    ...
    let must_line = text
        .lines()
        .find(|l| l.contains("return a token"))
        .expect("MUST clause line missing");
    let should_line = text
        .lines()
        .find(|l| l.contains("log the attempt"))
        .expect("SHOULD clause line missing");
    let may_line = text
        .lines()
        .find(|l| l.contains("set a cookie"))
        .expect("MAY clause line missing");

    assert!(
        must_line.contains("MUST"),
        "MUST keyword must appear on clause line"
    );
    assert!(
        must_line.contains("\u{2713}"),
        "passed indicator must appear on MUST line"
    );

    assert!(
        should_line.contains("SHOULD"),
        "SHOULD keyword must appear on clause line"
    );
    assert!(
        should_line.contains("\u{2717}"),
        "failed indicator must appear on SHOULD line"
    );

    assert!(
        may_line.contains("MAY"),
        "MAY keyword must appear on clause line"
    );
    assert!(
        may_line.contains("~"),
        "skipped indicator must appear on MAY line"
    );
}
    ...
    for line in text.lines() {
        if line.contains("clause-passed") {
            assert!(
                ...
                "passed clause line must show checkmark, got: {line:?}"
            );
        }
        if line.contains("clause-failed") {
            assert!(
                line.contains("\u{2717}"),
                "failed clause line must show X, got: {line:?}"
            );
        }
        if line.contains("clause-errored") {
            assert!(
                line.contains("!"),
                "errored clause line must show !, got: {line:?}"
            );
        }
        if line.contains("clause-absent") {
            assert!(
                line.contains("\u{2298}"),
                "absent clause line must show circle-slash, got: {line:?}"
            );
        }
        if line.contains("clause-skipped") {
            assert!(
                line.contains("~"),
                "skipped clause line must show ~, got: {line:?}"
            );
        }
    }
}
...
fn test_reporter__terminal_output__must_display_grouped_by_spec_section_clause() {
    let specs = vec![
        make_spec(
            ...
            vec![
                make_section(
                    "Login",
                    vec![make_clause(Keyword::Must, "issue JWT", "auth::login::jwt")],
                ),
                make_section(
                    "Logout",
                    vec![make_clause(
    ...
    assert!(
        pos_login < pos_logout,
        "Login section should appear before Logout section"
    );
    ...
    assert!(
        pos_login_header < pos_jwt_clause,
        "section header must precede its clauses"
    );
}
...
fn test_reporter__terminal_output__must_print_summary_line() {
    ...
    assert!(
        ...
        "summary must state '2 passed', got:\n{text}"
    );
    assert!(
        text.contains("1 failed"),
        "summary must state '1 failed', got:\n{text}"
    );
    assert!(
        text.contains("1 errored"),
        "summary must state '1 errored', got:\n{text}"
    );
    ...
    assert!(
        pos_clause < pos_passed,
        "clause content must appear before the summary line"
    );
}
...
fn test_reporter__terminal_output__must_color_code_by_severity() {
    ...
    let must_line = text
        .lines()
        .find(|l| l.contains("must-fail"))
        .expect("MUST failure line missing");
    let should_line = text
        .lines()
        .find(|l| l.contains("should-fail"))
        .expect("SHOULD failure line missing");
    let may_line = text
        .lines()
        .find(|l| l.contains("may-fail"))
        .expect("MAY failure line missing");
...
fn test_reporter__terminal_output__should_dim_passing_and_highlight_failures() {
    ...
    let passing_line = text_color
        .lines()
        .find(|l| l.contains("passing clause"))
        .expect("passing clause missing");
    assert!(
    ...
    let failing_line = text_color
        .lines()
        .find(|l| l.contains("failing clause"))
        .expect("failing clause missing");
    assert!(
...
fn test_reporter__terminal_output__wont_use_spinners_in_non_tty_mode() {
    ...
    assert!(
        ...
        "cursor-hide escape must not appear"
    );
    assert!(
        !text.contains("\x1b[1A"),
        "cursor-up escape must not appear"
    );
    assert!(
        !text.contains("\x1b[2K"),
        "erase-line escape must not appear"
    );
}
...
fn test_reporter__terminal_output__no_panic_with_missing_results() {
    ...
    let text = render_terminal(&run, &specs, &no_color_options());
    assert!(
        text.contains("no result for this"),
        "clause text should still appear"
    );
}
