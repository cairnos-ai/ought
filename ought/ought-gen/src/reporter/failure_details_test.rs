...
fn test_reporter__failure_details__should_show_clause_text_alongside_failure() {
    ...
    assert!(
        ...
        "output must show the original clause text; got:\n{text}"
    );
    // The failure message must also be present.
    assert!(
        text.contains("assertion failed"),
        "output must show the failure message; got:\n{text}"
    );
    ...
    let clause_pos = text
        ...
        .expect("clause text in output");
    let failure_pos = text
        .lines()
        .position(|l| l.contains("assertion failed"))
        .expect("failure message in output");
    assert!(
...
fn test_reporter__failure_details__must_show_file_and_line_from_failure_message() {
    let failure_msg =
        "thread 'test' panicked at tests/generated/api_response.rs:57:5:\nassertion failed";
