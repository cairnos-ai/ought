...
fn test_reporter__otherwise_chain__must_display_indented_under_parent() {
    ...
    let parent_pos = text
        .find("respond within 200ms")
        .expect("parent must be in output");
    let ow1_pos = text
        .find("return a cached response")
        .expect("first OTHERWISE must be in output");
    let ow2_pos = text
        .find("return 504")
        .expect("second OTHERWISE must be in output");
    ...
    let ow1_line = text
        .lines()
        .find(|l| l.contains("return a cached response"))
        .unwrap();
...
fn test_reporter__otherwise_chain__must_show_skipped_when_parent_passes() {
    ...
    let parent_line = text
        ...
        .unwrap();
    assert!(
        parent_line.contains("\u{2713}"),
        "passing parent must show checkmark, got: {parent_line:?}"
    );
    ...
    for clause_text in &["return a cached response", "return 504"] {
        ...
        assert!(
            ow_line.contains("~"),
            "OTHERWISE must show ~ when parent passes, got: {ow_line:?}"
        );
    }
}
