...
fn test_reporter__junit_xml__must_support_junit_path_flag() {
    ...
    assert!(
        out_path.exists(),
        "--junit must produce a file at the given path"
    );
    ...
    assert!(
        ...
        "JUnit XML must start with XML declaration"
    );
    assert!(
        contents.contains("<testsuites>"),
        "JUnit XML must contain <testsuites> root"
    );
...
fn test_reporter__junit_xml__must_map_specs_to_testsuites_and_clauses_to_testcases() {
    ...
    assert_eq!(
        suite_count, 2,
        "one <testsuite> per spec; found {suite_count}"
    );

    assert!(
        xml.contains("name=\"Spec Alpha\""),
        "first spec name must appear"
    );
    assert!(
        xml.contains("name=\"Spec Beta\""),
        "second spec name must appear"
    );

    let case_count = xml.matches("<testcase ").count();
    assert_eq!(
        case_count, 2,
        "one <testcase> per clause; found {case_count}"
    );

    assert!(
        xml.contains("classname=\"Spec Alpha\""),
        "classname must match spec name"
    );
...
fn test_reporter__junit_xml__must_include_failure_messages_and_clause_ids() {
    ...
    assert!(
        ...
        "a <failure> element must be present"
    );
    assert!(
...
fn test_reporter__junit_xml__should_include_keyword_and_severity_as_properties() {
    ...
    assert!(
        ...
        "<properties> element must be present"
    );
...
fn test_reporter__junit_xml__may_be_combined_with_terminal() {
    ...
    assert!(
        ...
        "JUnit XML must still be well-formed"
    );

    assert!(
        !text.is_empty(),
        "terminal output must not be empty when combined"
    );
