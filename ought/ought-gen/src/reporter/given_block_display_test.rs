...
fn test_reporter__given_block_display__must_display_given_as_visual_group() {
    ...
    given_clause.otherwise = vec![
        make_clause(
            ...
            "given::auth::must_profile",
        ),
        make_clause(
            Keyword::Must,
            "NOT return other users' data",
            "given::auth::must_not_leak",
        ),
    ];
    ...
    assert!(
        header_pos < clause1_pos,
        "GIVEN header must precede first child clause"
    );
    assert!(
        header_pos < clause2_pos,
        "GIVEN header must precede second child clause"
    );
}
