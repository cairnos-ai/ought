...
fn test_ought__implementation__should_use_a_workspace_structure_so_components_can_be_used_independ()
{
    ...
    assert!(
        members.len() >= 2,
        "Workspace should declare at least 2 members; found {} member(s).",
        members.len()
    );
...
fn test_ought__implementation__must_publish_the_spec_parser_as_a_standalone_crate_ought_spec_wit() {
    ...
    let spec_manifest =
        fs::read_to_string(spec_manifest_path).expect("failed to read ought-spec/Cargo.toml");
    ...
    for dep in &llm_deps {
        assert!(
            ...
            "ought-spec/Cargo.toml must not depend on LLM library '{}', but it was found in the manifest",
            dep
        );
    }
}
