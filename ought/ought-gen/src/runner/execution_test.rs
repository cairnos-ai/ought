...
fn test_runner__execution__must_support_running_tests_for_a_single_spec_file_filtering_gener() {
    ...
    let tmp = std::env::temp_dir().join(format!("ought_filter_spec_{}", std::process::id()));
...
fn test_runner__execution__must_by_complete_a_full_test_suite_execution_configurable_via_ought() {
        ...
        fn run(&self, tests: &[GeneratedTest], _test_dir: &Path) -> anyhow::Result<RunResult> {
            let results = tests
                ...
                .map(|t| TestResult {
                    ...
                    details: TestDetails {
                        measured_duration: Some(self.simulated_duration),
                        ..Default::default()
                    },
                })
                .collect();
    ...
    let tmp = std::env::temp_dir().join(format!("ought_mustby_{}", std::process::id()));
    std::fs::create_dir_all(&tmp).unwrap();

    let tests: Vec<GeneratedTest> = (0..5)
        .map(|i| GeneratedTest {
    ...
    let result = TimedRunner {
        simulated_duration: Duration::from_millis(1),
    }
    .run(&tests, &tmp)
...
fn test_runner__execution__must_not_trigger_generation_the_runner_only_executes_existing_generat() {
    ...
    let tmp = std::env::temp_dir().join(format!("ought_no_gen_{}", std::process::id()));
    std::fs::create_dir_all(&tmp).unwrap();
...
fn test_runner__execution__must_not_modify_generated_test_files_during_execution() {
    ...
    let tmp = std::env::temp_dir().join(format!("ought_immutable_{}", std::process::id()));
    std::fs::create_dir_all(&tmp).unwrap();
    ...
    let tests = vec![GeneratedTest {
        clause_id: ClauseId(
            "runner::execution::must_not_modify_generated_test_files_during_execution".to_string(),
        ),
    ...
    let after_bytes =
        std::fs::read(&test_file).expect("test file must still exist after runner execution");
    assert_eq!(
...
test_dir = "ought/ought-gen/"
"#;
    let parsed: RunnerOnly = toml::from_str(toml).expect("ought.toml must load without error");
...
";
    ...
    assert_eq!(
        results.len(),
        3,
        "must produce exactly one result per test line in harness output"
    );
...
fn test_runner__execution__must_pass_the_generated_test_files_directory_to_the_test_harness() {
    ...
    let test_dir = std::env::temp_dir().join(format!("ought_pass_dir_{}", std::process::id()));
    ...
    assert!(
        !paths.is_empty(),
        "runner must receive at least one test file path"
    );
    for p in &paths {
        assert!(
...
fn test_runner__execution__otherwise_kill_the_test_harness_process_and_report_a_timeout_error() {
    ...
    std::thread::spawn(move || {
        let tmp = std::env::temp_dir().join(format!("ought_kill_hang_{}", std::process::id()));
...
fn test_runner__execution__must_capture_stdout_stderr_and_exit_code_from_the_test_harness() {
    ...
    let tmp = std::env::temp_dir().join(format!("ought_capture_{}", std::process::id()));
    std::fs::create_dir_all(&tmp).unwrap();
    ...
    let result = failing
        .run(&tests, &tmp)
        .expect("runner must return Ok even on harness failure");
