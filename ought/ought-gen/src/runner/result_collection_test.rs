...
fn test_runner__result_collection__must_mark_remaining_lower_priority_otherwise_clauses_as_skipped_n()
 {
    ...
    let results = vec![
        ...
        OtherwiseResult {
            clause_id: "svc::must_use_replica_db".to_string(),
            priority: 1,
            status: Status::Failed,
        },
        OtherwiseResult {
            clause_id: "svc::must_use_cache".to_string(),
            priority: 2,
            status: Status::Passed,
        },
        OtherwiseResult {
            clause_id: "svc::must_use_fallback_response".to_string(),
            priority: 3,
            status: Status::Skipped,
        },
        OtherwiseResult {
            clause_id: "svc::must_return_503".to_string(),
            priority: 4,
            status: Status::Skipped,
        },
    ];
...
fn test_runner__result_collection__must_run_the_parent_test_first() {
    ...
    assert!(!log.is_empty());
    assert_eq!(
        log[0].clause_id, "auth::must_use_oauth",
        "parent must be first in execution log"
    );
    assert_eq!(
        log[0].execution_order, 0,
        "parent must have execution order 0"
    );
...
fn test_runner__result_collection__must_capture_failure_messages_assertion_errors_and_stack_traces_p()
 {
    ...
    let failure = ClauseResult {
        ...
        failure_message: Some(
            "thread 'test_must_debit_account' panicked at 'assertion `left == right` failed'"
                .to_string(),
        ),
        assertion_error: Some("left: 0\nright: 100".to_string()),
        stack_trace: Some(
            "stack backtrace:\n   0: std::panicking::begin_panic\n   1: test_must_debit_account"
                .to_string(),
        ),
    };
    ...
    assert!(
        failure.failure_message.is_some(),
        "failure message must be captured"
    );
    assert!(
        failure.assertion_error.is_some(),
        "assertion error detail must be captured"
    );
    assert!(
        failure.stack_trace.is_some(),
        "stack trace must be captured"
    );
...
fn test_runner__result_collection__must_stop_the_otherwise_chain_at_the_first_passing_fallback() {
    ...
    assert_eq!(
        results[1].status,
        Status::Failed,
        "fallback_0 fails, chain continues"
    );
    assert_eq!(
        results[2].status,
        Status::Passed,
        "fallback_1 passes, chain must stop here"
    );
    assert_eq!(
        results[3].status,
        Status::Skipped,
        "fallback_2 must be skipped — chain already stopped"
    );
}
