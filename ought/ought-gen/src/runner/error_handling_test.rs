...
fn test_runner__error_handling__must_report_when_a_generated_test_file_is_missing_referenced_in_m()
{
    ...
    let tmp = std::env::temp_dir().join(format!("ought_missing_file_test_{}", std::process::id()));
    ...
    assert_eq!(
        missing.len(),
        1,
        "Exactly one missing file must be reported"
    );
    assert_eq!(
        missing[0].clause_id, "runner::error_handling::clause_missing",
...
fn test_runner__error_handling__must_report_when_the_test_harness_command_is_not_found_or_fails_t()
{
    ...
    fn try_start_harness(binary: &str) -> Result<(), std::io::Error> {
        Command::new(binary).arg("--version").output().map(|_| ())
    }
...
fn test_runner__error_handling__must_always_leave_the_test_environment_clean_after_execution_no_leaked_c()
 {
    ...
    let base = std::env::temp_dir().join(format!("ought_invariant_{}", std::process::id()));
    ...
    for (i, &scenario) in scenarios.iter().enumerate() {
        ...
        assert!(
            !sentinel.exists(),
            "Temp sentinel file must be removed after scenario {:?}",
            scenario
        );
        assert!(
            !work_dir.exists(),
            "Work directory must be removed after scenario {:?}",
            scenario
        );
    }
    ...
    for iteration in 0u32..30 {
        ...
        assert!(
            !sentinel.exists(),
            "iteration {}: sentinel file leaked for scenario {:?}",
            iteration,
            scenario
        );
        assert!(
            !work_dir.exists(),
            "iteration {}: work dir leaked for scenario {:?}",
            iteration,
            scenario
        );
    }
...
fn test_runner__error_handling__should_detect_and_report_when_no_tests_were_generated_for_a_spec_no()
 {
    ...
    fn diagnose_run(spec: &str, generated_files: &[&str]) -> RunDiagnostic {
        if generated_files.is_empty() {
            RunDiagnostic::NoTestsGenerated {
                spec: spec.to_string(),
            }
        } else {
            RunDiagnostic::RanTests {
    ...
    assert!(
        ...
        "Runner must report 'no tests generated' when the file list is empty; got {:?}",
        diag
    );
    if let RunDiagnostic::NoTestsGenerated { spec } = &diag {
        assert_eq!(
            spec, "runner::error_handling",
            "Diagnostic must name the spec"
        );
    }
...
fn test_runner__error_handling__must_not_mask_harness_stderr_pass_it_through_for_debugging() {
    ...
    let output = Command::new("sh")
        .args([
            "-c",
            "echo 'harness diagnostic on stderr' >&2; echo 'stdout line'; exit 1",
        ])
        .stdout(Stdio::piped())
        .stderr(Stdio::piped())
        .output()
    ...
    assert!(
        ...
        "Captured stderr must contain the harness diagnostic; got: {:?}",
        stderr_text
    );
