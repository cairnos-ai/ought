...
fn test_mcp_server__resources__must_expose_ought_specs_list_of_all_spec_files_with_their_clause() {
    let _: fn(&ResourceHandler) -> anyhow::Result<serde_json::Value> = ResourceHandler::specs_list;
}
...
fn test_mcp_server__resources__must_expose_ought_coverage_clause_coverage_map_which_clauses_have() {
    let _: fn(&ResourceHandler) -> anyhow::Result<serde_json::Value> = ResourceHandler::coverage;
}
...
{
    let _: fn(&ResourceHandler) -> anyhow::Result<serde_json::Value> = ResourceHandler::manifest;
}
