...
fn test_generator__manifest_and_hashing__must_compute_a_clause_hash_from_the_keyword_clause_text_context_m()
 {
    ...
    let spec_must2 = OughtMdParser
        ...
        .expect("parse must succeed");
    assert_eq!(
        hash_must, spec_must2.sections[0].clauses[0].content_hash,
    ...
    let spec_other_text = OughtMdParser
        ...
        .expect("parse must succeed");
    assert_ne!(
        hash_must, spec_other_text.sections[0].clauses[0].content_hash,
...
fn test_generator__manifest_and_hashing__must_write_both_hashes_to_ought_ought_gen_manifest_toml_after_gen()
 {
    ...
    let tmp = std::env::temp_dir().join(format!("ought_both_hashes_{}", std::process::id()));
    ...
    manifest
        .save(&manifest_path)
        .expect("Manifest::save must succeed");

    // Raw TOML must contain both keys and their values.
    let toml_content =
        std::fs::read_to_string(&manifest_path).expect("manifest.toml must exist after save");
...
fn test_generator__manifest_and_hashing__must_record_the_model_name_and_timestamp_in_the_manifest_entry()
 {
    ...
    let tmp = std::env::temp_dir().join(format!("ought_model_ts_{}", std::process::id()));
    let _ = std::fs::remove_dir_all(&tmp);
    std::fs::create_dir_all(&tmp).unwrap();
    let manifest_path = tmp.join("manifest.toml");
    ...
    let content = std::fs::read_to_string(&manifest_path).expect("manifest.toml must be written");
...
fn test_generator__manifest_and_hashing__must_detect_and_remove_orphaned_generated_tests_clause_was_delete()
 {
    ...
    assert_eq!(
        ...
        3,
        "setup: manifest must start with three entries"
    );
