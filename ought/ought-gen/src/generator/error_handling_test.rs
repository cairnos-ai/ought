...
fn test_generator__error_handling__must_not_leave_the_manifest_in_an_inconsistent_state_if_generation_is()
 {
    ...
    let dir =
        std::env::temp_dir().join(format!("ought_manifest_consistency_{}", std::process::id()));
    ...
    manifest
        .save(&manifest_path)
        .expect("save after clause_a must succeed");
    ...
    assert!(
        loaded
            .entries
            .contains_key("generator::error_handling::clause_a"),
        "must_not_leave_manifest_inconsistent: committed clause_a must be present"
    );
    assert!(
        !loaded
            .entries
            .contains_key("generator::error_handling::clause_b"),
    ...
    // Idempotent reload
    let reloaded = Manifest::load(&manifest_path).expect("manifest must be loadable a second time");
    assert_eq!(
