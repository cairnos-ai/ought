...
fn test_mcp_server__resources__must_expose_ought_specs_list_of_all_spec_files_with_their_clause() {
    ...
    let base = std::env::temp_dir()
        .join(format!("ought_res_specs_{}", std::process::id()));
    ...
    // Verify ought://specs appears in the resources list
    ...
    assert!(
        list_resp.contains("ought://specs"),
        "resources/list must advertise 'ought://specs'; got: {list_resp}"
    );

    // Read the resource and verify it returns spec files with clause counts
    let resp = jsonrpc_exchange(&mut stdin, &mut reader,
        r#"{"jsonrpc":"2.0","id":3,"method":"resources/read","params":{"uri":"ought://specs"}}"#,
        5000);
    assert!(resp.is_some(), "ought://specs must respond to resources/read");
    let resp = resp.unwrap();
    assert!(
        resp.contains("\"result\""),
        "ought://specs must return a JSON-RPC result (not an error); got: {resp}"
    );
    assert!(
        resp.contains("auth") || resp.contains("clause") || resp.contains("count"),
        "ought://specs must include spec file names and clause counts; got: {resp}"
    );
