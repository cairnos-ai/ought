...
fn test_mcp_server__tools__must_expose_ought_status_return_spec_coverage_summary_clause_coun() {
    ...
    let base = std::env::temp_dir().join(format!("ought_tsta_{}", std::process::id()));
    ...
    assert!(
        ...
        "ought_status must return a JSON-RPC result; got: {resp}"
    );
    // The coverage summary must break down clause counts by severity (must/should/may) and status
    assert!(
        resp.contains("must") || resp.contains("severity") || resp.contains("required"),
        "ought_status result must include clause counts broken down by severity; got: {resp}"
    );
    assert!(
        resp.contains("pass") || resp.contains("fail") || resp.contains("status") || resp.contains("total"),
        "ought_status result must include clause counts broken down by status; got: {resp}"
    );
