...
fn test_mcp_server__tools__must_expose_ought_survey_analyze_source_for_uncovered_behaviors() {
    ...
    let base = std::env::temp_dir().join(format!("ought_tsur_{}", std::process::id()));
    ...
    std::fs::write(
        base.join("src").join("auth.rs"),
        "pub fn login(username: &str, password: &str) -> bool { !username.is_empty() && !password.is_empty() }\n",
    ).unwrap();
    ...
    // ought_survey must analyze the project source for behaviors not yet covered by specs
    let resp = jsonrpc_exchange(&mut stdin, &mut reader,
        r#"{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"ought_survey","arguments":{}}}"#,
        30000);
    assert!(resp.is_some(), "ought_survey must respond when called");
    let resp = resp.unwrap();
    assert!(
        resp.contains("\"result\""),
        "ought_survey must return a JSON-RPC result; got: {resp}"
    );
