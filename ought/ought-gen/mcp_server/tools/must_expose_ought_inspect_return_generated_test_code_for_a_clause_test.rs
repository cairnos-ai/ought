...
fn test_mcp_server__tools__must_expose_ought_inspect_return_generated_test_code_for_a_clause() {
    ...
    let base = std::env::temp_dir().join(format!("ought_tins_{}", std::process::id()));
    ...
    // ought_inspect must accept a clause_id and return the generated test code for that clause
    let resp = jsonrpc_exchange(&mut stdin, &mut reader,
        r#"{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"ought_inspect","arguments":{"clause_id":"auth::login::must_accept_valid_credentials"}}}"#,
        10000);
    assert!(resp.is_some(), "ought_inspect must respond when called with a clause_id");
    let resp = resp.unwrap();
    assert!(
        resp.contains("\"result\""),
        "ought_inspect must return a JSON-RPC result (not an error) for a known clause_id; got: {resp}"
    );
    // The result content must reference the test code for the clause
    assert!(
        resp.contains("code") || resp.contains("test") || resp.contains("fn "),
        "ought_inspect result must include the generated test code for the clause; got: {resp}"
    );
